...
/** 组装错误摘要：spawn 失败时 stderr/stdout 可能为 null，不得直接 .slice（防空指针掩蔽真实错误） */
const errSummary = (r) => String((r.stderr || r.stdout || `spawn 失败（status=${r.status} error=${r.error ?? ''}）`).slice(0, 400))
